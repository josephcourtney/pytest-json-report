...
def test_create_report(misc_testdir):
    misc_testdir.runpytest("--json-report", "-p", "pytest_json_report.plugin")
    assert (misc_testdir.tmpdir / ".report.json").exists()


def test_create_report_file_from_arg(misc_testdir):
    misc_testdir.runpytest("--json-report", "--json-report-file=arg.json", "-p", "pytest_json_report.plugin")
    assert (misc_testdir.tmpdir / "arg.json").exists()


def test_create_no_report(misc_testdir):
    misc_testdir.runpytest("--json-report", "--json-report-file=NONE", "-p", "pytest_json_report.plugin")
    assert not (misc_testdir.tmpdir / ".report.json").exists()


def test_terminal_summary(misc_testdir):
    res = misc_testdir.runpytest("--json-report", "-p", "pytest_json_report.plugin")
    res.stdout.fnmatch_lines(["-*JSON report*-", "*report saved*.report.json*"])

    res = misc_testdir.runpytest("--json-report", "--json-report-file=./", "-p", "pytest_json_report.plugin")
    res.stdout.fnmatch_lines(["*could not save report*"])

    res = misc_testdir.runpytest(
        "--json-report", "--json-report-file=NONE", "-p", "pytest_json_report.plugin"
    )
    res.stdout.no_fnmatch_line("-*JSON report*-")

    res = misc_testdir.runpytest(
        "--json-report", "--json-report-file=NONE", "-v", "-p", "pytest_json_report.plugin"
    )
    res.stdout.fnmatch_lines(["*auto-save skipped*"])

    res = misc_testdir.runpytest("--json-report", "-q", "-p", "pytest_json_report.plugin")
    res.stdout.no_fnmatch_line("-*JSON report*-")

    res = misc_testdir.runpytest(
        "--json-report", "-q", "--json-report-verbosity=0", "-p", "pytest_json_report.plugin"
    )
    ...
    res = misc_testdir.runpytest(
        ...
        "-vv",
        "--json-report-verbosity=0",
        "-p",
        "pytest_json_report.plugin",
    )
    res.stdout.no_fnmatch_line("-*JSON report*-")
...
def test_report_keys(num_processes, make_json):
    ...
    if num_processes < 2:
        expected_keys.add("collectors")
    if data.get("warnings"):
        expected_keys.add("warnings")
...
def test_process_report(testdir, make_json):
    ...
    testdir.makepyfile("""
        def test_foo():
            assert True
    """)
    res = testdir.runpytest("--json-report", "-p", "pytest_json_report.plugin")
    assert res.ret == 0


def test_indent(testdir, make_json):
    testdir.runpytest("--json-report", "-p", "pytest_json_report.plugin")
    with open(str(testdir.tmpdir / ".report.json"), encoding="utf-8") as f:
        assert len(f.readlines()) == 1
    testdir.runpytest("--json-report", "--json-report-indent=4", "-p", "pytest_json_report.plugin")
    with open(str(testdir.tmpdir / ".report.json"), encoding="utf-8") as f:
