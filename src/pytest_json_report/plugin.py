...
class JSONReportBase:
    ...
    def _validate_metadata(item):
        """Ensure that `item` has JSON-serializable metadata, otherwise delete it."""
...
class Hooks:
    def pytest_json_modifyreport(self, json_report):
        """Execute after building JSON report and before saving it.
    ...
    def pytest_json_runtest_stage(self, report):
        ...
        The report is the `_pytest.runner.TestReport` of the current test stage.
    ...
    def pytest_json_runtest_metadata(self, item, call):
        """Return a dict which will be added to the current test item's JSON metadata.

        Called from `pytest_runtest_makereport`. Plugins can use this hook to add metadata based on the
        current test run.
        """
...
def pytest_configure(config):
    if not config.option.json_report:
        return
    Plugin = JSONReportWorker if hasattr(config, "workerinput") else JSONReport  # noqa: N806
